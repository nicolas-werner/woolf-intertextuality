...
class TokenCounter:
    ...
    def track_completion(self, messages: List[Dict[str, str]], completion_tokens: int, cached: bool = False):
        ...
        input_price = (
            self.PRICING['gpt-4o']['cached_input'] if cached 
    ...
    def print_usage_report(self):
        ...
        console.print(f"Standard Input tokens: {self.usage['completion']['input_tokens']:,}")
        console.print(f"Cached Input tokens: {self.usage['completion']['cached_input_tokens']:,}")
        ...
        batch_cost = (
            ((self.usage['completion']['input_tokens'] + 
              self.usage['completion']['cached_input_tokens']) / 1000 * 0.00125) +
